...
qtlrow=qtl[qtl$environment==env & qtl$phenotype==pheno & qtl$ID==id,]
cov_snp=qtlrow$SNP
cov_chr=qtlrow$CHR
#cov_snp="AX-90834914"
#cov_chr="3"

X_list_cov=readRDS(sprintf('../genotypes/probabilities/geno_probs/bg%s_filtered_genotype_probs.rds',cov_chr))
X_cov=do.call(cbind,lapply(X_list_cov,function(x) x[inter,cov_snp]))
...
data=data[inter,]
# Run GridLMM
null_model = GridLMM_ML(y~0+X_cov+(1|ID),data,relmat=list(ID=K),ML=T,REML=F,verbose=F)
...
for(g in fgroups){
	...
	subfkeep=fkeep[,subm,drop=F]
  	...
  	if(g==16){
      	...
      	endp=6+length(allcov)-1
		end=endp+(length(founders)-1)
      	names(gwas)[6:endp]=allcov
		names(gwas)[(endp+1):end]=founders[-1]
		gwas[,allcov[-1]]=gwas[,allcov[1]] + gwas[,allcov[-1]]
		gwas[,founders[1]]=apply(gwas[,allcov],MARGIN=1,mean)
        new_gwas=gwas[,c('Trait','X_ID','s2','ML_logLik','ID.ML',allcov,founders,'n_steps','Df_X','ML_Reduced_logLik','Reduced_Df_X','p_value_ML')]
...
fwrite(all_gwas,sprintf('QTL/covariate/Biogemma_chr%s_%s_x_%s_%s_covariate.txt',chr,pheno,env,id),row.names=F,quote=F,sep='\t')
