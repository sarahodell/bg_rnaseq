...
pdf(sprintf('eqtl/images/%s_cis_eqtl_by_ind.pdf',time))
...
for(i in 1:nrow(ciseqtl)){
  ...
  inter=intersect(exp$V1,dimnames(X_list[[1]])[[1]])
  norm=norm[inter,]
  snp=ciseqtl[i,]$SNP
  ...
  exp=exp[inter,]
  ex=data.frame(ID=exp$V1,exp=exp[,gene],founder=founder,stringsAsFactors=F)
  ex=ex[ex$ID!="",]
  ex=ex[order(ex$exp),]
...
pdf(sprintf('eqtl/images/%s_founder_ciseqtl.pdf',time))
for(i in 1:length(plot_list)){
  print(plot_list[[i]])
}
dev.off()

pdf(sprintf('eqtl/images/%s_founder_ciseqtl_by_ind.pdf',time))
for(i in 1:length(plot_list2)){
  print(plot_list2[[i]])
}
dev.off()

# Do these cis-eQTL genes load onto any factors?
gene="Zm00001d031961" #WD_0712
gene="Zm00001d025017" #WD_0720

times=c("WD_0712","WD_0718","WD_0720","WD_0727")

for(time in times){
	prop_var=fread(sprintf('MegaLMM/MegaLMM_%s_prop_variance.txt',time),data.table=F)
	l=prop_var[prop_var$V1==gene,]
	print(time)
	print(l)
