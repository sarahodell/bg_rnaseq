...
df$log10qvalue=-log10(df$p_adjusted)
...
sig=df[df$log10qvalue>=threshold,]
sig=sig[order(sig$CHR,sig$BP),]
df$log10p=-log10(df$p_value_ML)
names(df)=c("Trait","X_ID","p_value_ML","time","factor","CHR","BP","p_adjusted","log10qvalue",'value')
...
fwrite(subdf,sprintf('paper_figures/%s_%s_factor_transeqtl_pvalues.txt',time,factor),row.names=F,quote=F,sep='\t')
...
for(time in times){
	...
	#threshold=-log10(0.1)
	threshold=min(subdf[subdf$log10qvalue>=1,]$value)

	for(factor in sig_factors){
		subdf2=subdf[subdf$factor==factor,]
		ymax=round(max(subdf$value))+1
		title=sprintf("TGE  %s %s trans-eQTL",factor,time)
...
pdf('paper_figures/trans_eQTL_manhattan_plots.pdf')
...
for(time in times){
	...
	for(factor in sig_factors){
		subdf2=subdf[subdf$factor==factor,]
		ymax=round(max(subdf$value))+1
		title=sprintf("DRE %s %s trans-eQTL",factor,time)
		a2<-gg.manhattan2(fdf=subdf2,threshold,
             col=greypalette,
             ylims=c(0,ymax)) + labs(caption = title)
		plist[[count]]=a2
...
pdf('paper_figures/DRE_trans_eQTL_manhattan_plots.pdf')
for(i in 1:length(plist)){
	print(plist[[i]])
}
dev.off()
