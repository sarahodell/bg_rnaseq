...
df=c()
for(c in 1:10){
  d=fread(sprintf('eqtl/results/eQTL_%s_c%.0f_results.txt',time,c))
...
gg.manhattan2 <- function(df, threshold, col, ylims,bounds){
  ...
  ggplot(df.tmp, aes(x=BPcum, y=value)) +
    ...
    geom_point(data=subset(df.tmp, sig==T), color="coral2", size=3) +
...
png(sprintf('eqtl/images/ciseQTL_manhattan_%s.png',time),width=2000,height=1500)
print(a2)
dev.off()
