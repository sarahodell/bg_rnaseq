...
df=c()
for(c in 1:10){
  d=fread(sprintf('eqtl/results/%s_c%s_%s_eQTL.txt',time,c,pc))
...
png(sprintf('eqtl/cis/images/%s_%s_fkeep_qqplot.png',time,pc))
...
png(sprintf('eqtl/images/%s_manhattan_%s.png',pc,time),width=2000,height=1500)
print(a2)
dev.off()

sigs=df[df$value>=threshold,]
fwrite(sigs,sprintf('eqtl/results/%s_%s_hits.txt',time,pc),row.names=F,quote=F,sep='\t')
