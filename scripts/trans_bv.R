...
# Only get bvs for genes that are on t chr
n_reps=match(cgenes,tgenes)
#n_reps=1:length(tgenes)
#n_reps=1:5

print(system.time({
full_results=mclapply(n_reps,get_allbv,mc.cores=cores)
}))

saveRDS(full_results,sprintf('eqtl/trans/results/trans_eQTL_%s_c%s_bvs_FIXED.rds',time,t_chr))

#d=rbindlist(dlist)
#d=as.data.frame(d)

#fwrite(full_results,sprintf('eqtl/trans/results/trans_eQTL_%s_c%s_weights_results_filtered_FIXED.txt',time,t_chr),row.names=F,quote=F,sep='\t')
