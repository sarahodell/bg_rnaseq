...
for(c in 1:10){
  ...
  d=fread(sprintf('eqtl/trans/results/%s_c%.0f_%s_trans_results.txt',time,c,factor))
...
if(dim(sigs2)[1]!=0){
  ...
  png(sprintf('eqtl/trans/images/%s_trans_eQTL_fkeep_manhattan_%s.png',factor,time),width=2000,height=1500)
  print(a2)
  dev.off()
  fwrite(sigs2,sprintf('eqtl/results/%s_trans_%s_eQTL_fkeep_hits.txt',factor,time),row.names=F,quote=F,sep='\t')
