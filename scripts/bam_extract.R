...
fwrite(data,sprintf('final_bams/%s_path_samples.txt',time),row.names=F,quote=F,sep=',',col.names=F)
...
#WD_0720
addons=data.frame(Gene_ID=c("Zm00001d010752","Zm00001d010987","Zm00001d010988","Zm00001d042315"))
genebed=rbind(genebed,addons)
if(time=="WD_0720"){
	genebed=rbind(genebed,c('Zm00001d025016'))
...
fwrite(genebed,sprintf('eqtl/results/%s_genes.bed',time),row.names=F,quote=F,col.names=F,sep='\t')
...
genebed=fread(sprintf('eqtl/results/%s_genes.bed',time),data.table=F)
...
for(i in 1:nrow(genebed)){
	...
	fwrite(newdf,sprintf('eqtl/results/%s_%s_founder_ident.txt',time,gene),row.names=F,quote=F,sep='\t')
}
...
fullbed=c()
for(time in times){
	bed=fread(sprintf('eqtl/results/%s_genes.bed',time),data.table=F)
	bed$time=time
	fullbed=rbind(fullbed,bed)
}

fullbed=as.data.frame(fullbed,stringsAsFactors=F)
fwrite(fullbed,'eqtl/results/all_eQTL_genes.bed',row.names=F,quote=F,col.names=F,sep='\t')
...
fwrite(subgtf,'eqtl/results/all_eQTL_genes.gtf',row.names=F,quote=F,sep='\t',col.names=F)
...
genebed=fread(sprintf('eqtl/results/%s_genes.bed',time),data.table=F)
data=fread(sprintf('final_bams/%s_path_samples.txt',time),data.table=F,header=F)
for(i in 1:nrow(genebed)){
	row=genebed[i,]
	query=paste0(row$V1,':',row$V2,'-',row$V3)
	gene=row$V4
	newdf=fread(sprintf('eqtl/results/%s_%s_founder_ident.txt',time,gene),data.table=F)
