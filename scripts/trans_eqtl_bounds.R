			...
			subtrans=subtrans0[subtrans0$Trait==gene,]
 			while(nrow(subtrans)!=0){
  				if(nrow(subtrans)==1){
					...
					snp=startrow$X_ID
					max_pos=startrow$BP
					window_range=c(max_pos-2.5e7, max_pos + 2.5e7) # Within 20Mb of highest SNP
		...
		if(nrow(subtrans0)!=0){
			...
			for(i in 1:nrow(subtrans0)){
				...
				rownames(window)=seq(1,dim(window)[1])
  				...
  				bounds[bounds$time==time & bounds$gene==gene & bounds$SNP==snp,]$left_bound_bp=left_bound_bp
  				bounds[bounds$time==time & bounds$gene==gene & bounds$SNP==snp,]$left_bound_snp=left_bound_snp
  				bounds[bounds$time==time & bounds$gene==gene & bounds$SNP==snp,]$right_bound_bp=right_bound_bp
		  		bounds[bounds$time==time & bounds$gene==gene & bounds$SNP==snp,]$right_bound_snp=right_bound_snp
		  		bounds[bounds$time==time & bounds$gene==gene & bounds$SNP==snp,]$alt_right_bound_bp=all_founder_blocks[all_founder_blocks$focal_snp==snp,]$end
			}
		}	
	}
}
