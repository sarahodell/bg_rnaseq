...
for(c in 1:10){
  ...
  d=fread(sprintf('eqtl/trans/results/%s_c%.0f_pheno_%s_trans_results.txt',time,c,factor))
...
if(dim(sigs2)[1]!=0){
  ...
  png(sprintf('eqtl/trans/images/%s_pheno_trans_eQTL_fkeep_manhattan_%s.png',factor,time),width=2000,height=1500)
  print(a2)
  dev.off()
